...
The user interface is an inline-button menu. `/start` only opens the menu. At startup, the bot clears its Telegram slash-command list. Customers can browse paid and test plans, submit a single-user daily reseller trial, buy from wallet or by direct payment, view wallet and ledger, request top-ups and submit receipt photos, and inspect owner-scoped subscription details and links before confirming cancellation. Menu visibility, labels, and the home title read the deployment's backend feature and text settings. Batch reseller trials remain disabled. Payment and trial decisions remain backend-authoritative.

Telegram user ID `96937669` opens the hidden admin menu with `/admin` in this reseller deployment. The admin menu is omitted from `/start`; the bot also clears Telegram's published command list, so regular users do not see `/admin` in the command picker. Both the configured Telegram ID and the backend-confirmed admin role are required to open it. The menu supports reseller application approval or rejection, pending payment and top-up approvals, plan creation and field-by-field plan editing, payment instructions, reseller trial quota and approval settings, feature switches, user-facing bot text, and deployment-scoped panel configuration. The backend authorizes every admin API request. Panel tokens are submitted directly to the backend and the bot deletes the Telegram message containing the token after handling it.
